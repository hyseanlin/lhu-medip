...
N = 5
blur_avg = cv2.blur(noise_img, (N, N)) #N設為13 
blur_gauss = cv2.GaussianBlur(noise_img, (N, N), 0) #N設為13 
blur_med = cv2.medianBlur(noise_img , N, 0) #N設為13 


cv2.imshow('Original', img)
cv2.imshow('Salt & Pepper Noise', noise_img)
cv2.imshow('Image filtered by Averaging Filter', blur_avg)
cv2.imshow('Image filtered by Gaussian Filter', blur_gauss)
cv2.imshow('Image filtered by Median Filter', blur_med)
cv2.waitKey(0)
